...
import { TypeTestSchema, TypeTestOfListDiscipline, TypeTestsPerTerms, TypeTestsPerTeachers, TypeTestOfListTeacher } from "../types/testType";
...
export async function listTestsPerTeacher() {
    ...
    const rawTests: TypeTestsPerTeachers[] = await testRepository.findTestsPerTeacher();

    for (const teacher of rawTests) {
        
        for (const teacherDiscipline of teacher.teacherDisciplines) {
            
            if (teacherDiscipline.tests.length === 0) continue;
            const discipline =teacherDiscipline.discipline;
            delete teacherDiscipline.discipline;

            const hashtableCategory: IHashTableCategory<TypeTestOfListTeacher> = {};

           
            for (const test of teacherDiscipline.tests) {
                
                const newTest: TypeTestOfListTeacher = { ...test }
                    newTest.discipline = discipline;
                    if (!hashtableCategory[newTest.category.name]) {
                        hashtableCategory[newTest.category.name] = [newTest];
                        continue;
                    }
